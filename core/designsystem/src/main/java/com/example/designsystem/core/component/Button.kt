 ...
 */
@Composable
fun DealFilledButton(
    onClick: () -> Unit,
    modifier: Modifier = Modifier,
    enabled: Boolean = true,
    small: Boolean = false,
    colors: ButtonColors = DealButtonDefaults.filledButtonColors(),
    contentPadding: PaddingValues = DealButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    Button(
        onClick = onClick,
        modifier = if (small) {
            modifier.heightIn(min = DealButtonDefaults.SmallButtonHeight)
        } else {
            modifier
        },
        enabled = enabled,
...
fun DealFilledButton(
    ...
    DealFilledButton(
        ...
        small = small,
        colors = colors,
        contentPadding = DealButtonDefaults.buttonContentPadding(
            small = small,
...
fun DealOutlinedButton(
    ...
    enabled: Boolean = true,
    small: Boolean = false,
    border: BorderStroke? = DealButtonDefaults.outlinedButtonBorder(enabled = enabled),
    colors: ButtonColors = DealButtonDefaults.outlinedButtonColors(),
    contentPadding: PaddingValues = DealButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    OutlinedButton(
...
fun DealTextButton(
    ...
    enabled: Boolean = true,
    small: Boolean = false,
    colors: ButtonColors = DealButtonDefaults.textButtonColors(),
    contentPadding: PaddingValues = DealButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    TextButton(
 ...
 */
@Composable
private fun RowScope.DealButtonContent(
    text: @Composable () -> Unit,
    leadingIcon: @Composable (() -> Unit)?,
    trailingIcon: @Composable (() -> Unit)?
) {
    if (leadingIcon != null) {
        Box(Modifier.sizeIn(maxHeight = DealButtonDefaults.ButtonIconSize)) {
            leadingIcon()
        }
    }
    Box(
        Modifier
            .padding(
                start = if (leadingIcon != null) {
                    DealButtonDefaults.ButtonContentSpacing
                } else {
                    0.dp
                },
